...
= 演習の使用部品
== @H_加速度センサ @加速度センサ の電子部品 ( 加速度センサ KXM52-1050) を次のような点から調べなさい。 <H_加速度センサ>
...
==== 回路図 <H_実験1回路図>
@H_実験1回路図 @実験1回路図 は、実験その1の回路図を示す。
...
==== プログラム <H_実験1プログラム>
@H_実験1プログラム @値の取得をするソースコード1 , @H_実験1プログラム @値の取得をするソースコード2 は、実験その1で使用したアナログ入力値を取得するソースコードを示す。
...
今回の実験の軸の定義としては @H_実験1のグラフ @加速度センサの軸 のように定義する。
...
@H_実験1のグラフ @加速度センサを傾けた時のグラフ を見ると、X軸、Y軸、Z軸の加速度をそれぞれ測定できる。

==== 結果

値はブレッドボードの角度の変化をした場合その角度の変化を検知して値が変化する。 
角度を変化させず、動作をさせず、水平に保った場合はx軸,y軸は500前後の値を示す。z軸は750前後の値を示す。
角度の瞬間的な変化を出力するのではなく、値は前のデータを保持した値となり、角度を水平にしない限り最初の値に戻らない。 
...
卓上に置いた時にz軸の値だけ大きくなっているのはセンサに掛かっている力が重力だけでz軸に対して掛かっているためである。 
...
実験結果は、@H_実験2のグラフ @加速度センサを動かした時のグラフ に示す。

#figure(
  image("./img/実験2のグラフ.png",width: 60%),
  caption: "加速度センサを動かした時のグラフ"
)<加速度センサを動かした時のグラフ>

@加速度センサを動かした時のグラフ を見ると、X軸、Y軸、Z軸の加速度をそれぞれ測定できる。
...
=== 発展その1 (傾きを用いたアニメーション制御) <H_発展その1の加速度センサの動作イメージ>
@H_発展その1の加速度センサの動作イメージ @発展その1の加速度センサの動作イメージ @H_発展その1の加速度センサの動作イメージ @発展その1の記号の動作イメージ のように，加速度センサを用いて，傾けた向きに傾きの大きさだけ記号が動く(加速度センサの傾きをトレースする) ようにしなさい．また，平らな場所に置くと記号が中央に戻るようにしなさい．
...
@H_発展1のプログラム @加速度センサの傾きを用いたアニメーション制御のソースコード は、加速度センサの傾きをトレースするソースコードを示す。
...
@H_発展2のプログラム @移動速度を用いたアニメーション制御のソースコード1 , @H_発展2のプログラム @移動速度を用いたアニメーション制御のソースコード2 は、移動速度を用いたアニメーション制御のソースコードを示す。

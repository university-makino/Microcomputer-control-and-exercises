...
したがって、光の量によって放出する自由電子の量が変化し、抵抗値が変化する。
...
特に抵抗のように動作をするためピンアサインなどはないが、@CdsStructure 構造図を示すと、2つの端子がある@cds_datasheet 。
...
秋月電子によると、GL5528の仕様は以下の通りである@cds_akituki 。
...
- プログラムの説明
  - 1–7 行目: プログラムに必要な変数の宣言および定義またはライブラリのインポート
...
@マイコンの値と照度計の値の特性グラフ では、実験1におけるマイコンへのアナログ入力値と照度計の値の特性グラフを示している。
グラフより、Cdsセルは、明るい状況下では抵抗値が低くなり、暗くなると抵抗値が上昇する。また感度は非常に高く、強い光を当てても抵抗値が完全に0にはならない。逆に、非常に暗い状況でも抵抗値が無限大にならない。また、センサーの反応速度は非常に速い。
...
また、センサーの特性は、対数関係を持っているため、マイコンのアナログ入力値をLuxに変換できる。
...
グラフより、実験1とは動作が異なり、明るい状況下ではマイコンにかかる電圧が上昇し、暗い状況下ではマイコンにかかる電圧が減少する。それ以外の動作については実験1と同様である。
...
抵抗分圧とは、電圧を分圧するために抵抗を使用する回路である。 
抵抗分圧回路は、2つの抵抗を直列に接続し、その間に電圧をかけると、電圧が分圧される。 

...
=== アナログ
アナログは、連続した量を他の連続した量で表示すること。デジタルが連続量を離散的な数値として表現（標本化・量子化）することと対比される。

=== デジタル
デジタルとは、量を段階的に区切って数字で表すことや、情報を離散的な値つまり飛び飛びの値のあつまりとして表現し"段階的な "物理量に対応させて記憶・伝送する方式や、データを "有限桁の数値" で表現する方法であり、たとえば 0と1だけを有限個使って情報を伝えることである。

=== センサ
センサは、自然現象や人工物の機械的・電磁気的・熱的・音響的・化学的性質あるいはそれらで示される空間情報・時間情報を、何らかの科学的原理を応用して、人間や機械が扱い易い別媒体の信号に置き換える装置のことをいう。

=== アクチュエータ
電気・空気圧・油圧などのエネルギーを機械的な動きに変換し、機器を正確に動かす駆動装置。

=== LED, 極性
ダイオードの1種で、順方向に電圧を加えた際に発光する半導体素子である。発光原理にはエレクトロルミネセンス効果を利用している。
LEDには極性があり、アノード（+）とカソード（-）がある。アノードに正の電圧を加えると発光する。
見分け方は、アノードには長い足がついている。また、LEDが平らな場合は、平らな側がカソードである。

=== 回路図，実体配線図
回路図とは、電気回路や空気圧機器、油圧機器などの回路を記載する図を指す。
